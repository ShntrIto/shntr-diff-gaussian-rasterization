...
class _RasterizeGaussians(torch.autograd.Function):
    ...
    def forward(
        ...
        if raster_settings.spherical:
            # modal
            num_rendered, color, radii, out_all_modal, out_plane_depth, geomBuffer, binningBuffer, imgBuffer = _C.rasterize_spherical_gaussians(*args)
        else:
        ...
        ctx.save_for_backward(out_all_modal, colors_precomp, all_modal, means3D, scales, rotations, cov3Ds_precomp, radii, sh, geomBuffer, binningBuffer, imgBuffer)
        return color, radii, out_all_modal, out_plane_depth

    @staticmethod
    def backward(ctx, grad_out_color, _, grad_out_all_modal, grad_out_plane_depth):

        # Restore necessary values from context
        num_rendered = ctx.num_rendered
        raster_settings = ctx.raster_settings
        all_modal_pixels, colors_precomp, all_modal, means3D, scales, rotations, cov3Ds_precomp, radii, sh, geomBuffer, binningBuffer, imgBuffer = ctx.saved_tensors
